...
namespace OpenPay
{
    public class OpenpayFeesService
    {
        ...
        public async Task<OpenpayFeesSummary> Summary(int year, int month)
        {
            string url = GetEndPoint();
            url = url + BuildParams(year, month, null, null);
            return await this.httpClient.Get<OpenpayFeesSummary>(url);
        }

        public async Task<List<Transaction>> Details(int year, int month, string fee_type, PaginationParams paginationParams)
        {
            string url = GetEndPoint() + "/detail";
            url = url + BuildParams(year, month, fee_type, paginationParams);
            return await this.httpClient.Get<List<Transaction>>(url);
        }
